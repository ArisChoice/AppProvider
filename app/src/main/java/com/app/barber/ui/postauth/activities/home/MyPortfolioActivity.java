...
public class MyPortfolioActivity extends BaseActivity implements HomeAuthMVPView {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        imgEdit.setImageResource(R.drawable.add_copy);
        initRecyclar();
        getMyStyleImages();
    }
    ...
    private void initRecyclar() {
        myPortfolioAdapter = new MyPortfolioAdapter(MyPortfolioActivity.this, ProfileScreenActivity.class.getName(), new ArrayList<MyImagesResponseModel.ImagesBean>(),
        ...
        recyclarViewLst.setLayoutManager(new GridLayoutManager(MyPortfolioActivity.this, 3, GridLayoutManager.VERTICAL, false));
    ...
    private void openPickAlert(int i) {
        ...
        CommonUtils.getInstance(MyPortfolioActivity.this).openDialog(this, name, icons, new OnBottomDialogItemListener() {
            @Override
            public void onItemClick(View view, int position, int type, Object t) {
                switch (type) {
...
//      mCurrentCameraPhotoPath = f.getAbsolutePath();
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            Uri photoURI = FileProvider.getUriForFile(MyPortfolioActivity.this, BuildConfig.APPLICATION_ID + ".provider", file);
    ...
    private void cropRequest(String outputFile) {
        CropImage.activity(Uri.fromFile(new File(outputFile))).setAspectRatio(1,
                1).setFixAspectRatio(true).start(MyPortfolioActivity.this);
        ...
        switch (serviceMode) {
            case NetworkConstatnts.RequestCode.API_POST_WORKSPACE_IMAGES:
                if (((BaseResponse) model).getStatus() == NetworkConstatnts.ResponseCode.success) {
                    new CommonUtils().displayMessage(MyPortfolioActivity.this, ((BaseResponse) model).getMessage());
                    getMyStyleImages();
                } else
                    new CommonUtils().displayMessage(MyPortfolioActivity.this, ((BaseResponse) model).getMessage());
                break;
