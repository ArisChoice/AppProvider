...
public class AddWorkspacePhotos extends BaseActivity implements BasicAuthMVPView {
    ...
    @BindView(R.id.with_image_layout)
    LinearLayout withImageLayout;
    @BindView(R.id.change_image_btn)
    CustomTextView changeImageBtn;
    private SelectedImagesAdapter imageAdapter;
    private BssicAuthPresenter presenter;
    private ImagePickerModel pickedImage;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        firstHeaderTxt.setText(R.string.workspace_);
        secondHeaderTxt.setText(R.string.workspace_text);
    ...
    @OnClick({R.id.continue_btn, R.id.add_photo_btn, R.id.change_image_btn})
...
//                activitySwitcher(AddWorkspacePhotos.this, HomeActivity.class, null);
    ...
    private void saveImages() {
        ...
        // add_ Parameter
        params.put(NetworkConstatnts.Params.value, RequestBody.create(MediaType.parse("text/plain"), GlobalValues.ImageTypeKey.BANNER_IMAGE));
        if (pickedImage != null) {
            File file = new File(pickedImage.getImagePath());
...
//      mCurrentCameraPhotoPath = f.getAbsolutePath();
        ...
        if (requestCode == GlobalValues.RequestCodes.REQUEST_TAKE_IMAGE && resultCode == RESULT_OK) {
            ...
            if (resultCode == RESULT_OK) {
             ...
             /*   ArrayList<ImagePickerModel> imageList = new ArrayList<>();
                ...
                notifiScreenView();*/
                notifiScreenView();
                bannerImage.setImageURI(resultUri);
    ...
    private void notifiScreenView() {
        if (pickedImage.getImagePath() != null && !pickedImage.getImagePath().equals("")) {
            noImageLayout.setVisibility(View.GONE);
            withImageLayout.setVisibility(View.VISIBLE);
        } else {
            noImageLayout.setVisibility(View.VISIBLE);
            withImageLayout.setVisibility(View.GONE);
        }
    ...
    private void cropRequest(String outputFile) {
        CropImage.activity(Uri.fromFile(new File(outputFile))).setAspectRatio(16,
                9).setFixAspectRatio(true).start(AddWorkspacePhotos.this);
        ...
        switch (serviceMode) {
            case NetworkConstatnts.RequestCode.API_POST_WORKSPACE_IMAGES:
                if (((BaseResponse) model).getStatus() == NetworkConstatnts.ResponseCode.success) {
                    ...
                    activitySwitcher(AddWorkspacePhotos.this, DisplayImageActivity.class, null);
                } else
                    new CommonUtils().displayMessage(AddWorkspacePhotos.this, ((BaseResponse) model).getMessage());
                break;
        }
