...
public abstract class BaseFragment extends Fragment {
    ...
    public void onMessage(CustomEvent event) {
        ...
        for (int i = 0; i < cFragment.size(); i++) {
            if (cFragment instanceof AppointsmentsFragment) {
                AppointsmentsFragment.getInstance().onMessage(event);
            } else if (cFragment instanceof DashboardFragment) {
                DashboardFragment.getInstance().onMessage(event);
            }
        }
    }
